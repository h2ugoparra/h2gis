...
Convert SST downloads from the default downloads directory (DOWNLOADS_DIR/.env):
    ...
    uv run h2gis convert -v sst

Convert multiple variables in one call:

    uv run h2gis convert -v sst -v ssh -v mld

Convert files from a custom input directory:

    uv run h2gis convert -v sst --in-dir /data/raw/CMEMS_SST
...
- Provenance sidecars (``*_prov.json``) are written alongside each Zarr
  only when a ``h2gis_manifest.json`` exists in the input directory (created
...
from h2gis.format_converters.netcdf2zarr import Netcdf2Zarr
